...
void Complementary_Filter::Compute(IMU::Sensor_Data accel, IMU::Sensor_Data gyro, IMU::Euler_Angles& euler) {
	...
	this->euler.roll = this->COMPLEMENTARY_COEFFICIENT * (this->euler.roll + gyro.y * this->DELTA_T) + (1 - this->COMPLEMENTARY_COEFFICIENT) * accel_roll;

	this->euler.pitch = this->COMPLEMENTARY_COEFFICIENT * (this->euler.pitch + gyro.x * this->DELTA_T) + (1 - this->COMPLEMENTARY_COEFFICIENT) * accel_pitch;
